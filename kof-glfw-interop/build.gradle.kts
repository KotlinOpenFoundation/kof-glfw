  ...
  targets.withType<KotlinNativeTarget>().configureEach {
    binaries.all {
      ...
      when (konanTarget.family) {
        ...
        Family.OSX -> {
          val arch = if (konanTarget.architecture == Architecture.ARM64) "lib-arm64" else "lib-x86_64"
          linkerOpts("-L$depsDir/macos/$arch", "-Wl,-rpath,@executable_path")
        }
        Family.LINUX -> {
          val arch = if (konanTarget.architecture == Architecture.ARM64) "lib-arm64" else "lib-x64"
...
tasks {
  ...
  val cMakeTargets = listOf(
    CMakeCompileTarget(Family.LINUX, Architecture.X64),
    CMakeCompileTarget(
      Family.LINUX,
      Architecture.ARM64,
    ...
    doLast {
      listOf("lib-x86_64", "lib-arm64").forEach { arch ->
        ...
        if (!Files.exists(link)) {
          Files.createSymbolicLink(link, Path.of("libglfw.3.dylib"))
        }
      }
    }
  }
  ...
  withType<KotlinNativeLink> {
    val nativeBinary = binary ?: return@withType
    val konanTarget = nativeBinary.compilation.konanTarget
    ...
    val outputDir = nativeBinary.outputDirectory
    doLast {
      when (konanTarget.family) {
        Family.MINGW -> {
